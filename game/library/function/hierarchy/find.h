...
static inline struct ipo* ipo_find(const char* name, const struct superobject* root)
{
    if (!root) return NULL;
    struct ipo* ipo = (struct ipo*)superobject_data(root);
    if ((superobject_type(root) == superobject_type_ipo || superobject_type(root) == superobject_type_ipo2) && ipo)
    {
