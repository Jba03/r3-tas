...
{ \
    ...
    tok->node = Node; \
    tok->offset = 0; \
    if (Node) tok->offset = (int)(((library::stNodeInterpret*)Node) - t->tree); \
    t->n_tokens++; \
}
...
{
    uint32_t param = t->current->param;
    if (param == 0) { /* If */
        emit("if ")
        //emit2("(")
...
{
    uint32_t param = (t->current->param);
    const char* condition = R3ConditionTable[param].c_str();
...
{
    uint32_t param = (t->current->param);
    const char* function = R3FunctionTable[param].c_str();
...
{
    uint32_t param = (t->current->param);
    const char* proc = R3ProcedureTable[param].c_str();
    emit("%s", proc)
    emit2("(")
    translate_branch(t, true);
    emit2(");")
...
{
    uint32_t param = (t->current->param);
    const char* meta_action = R3MetaActionTable[param].c_str();
    emit("%s", meta_action)
    emit2("(")
    translate_branch(t, true);
    emit2(");")
...
{
    uint32_t param = (t->current->param);
    const char* field = R3FieldTable[param].c_str();
    emit("%s", field)
}
...
{
    uint32_t intrm = (t->current->param);
...
static void translate_childnode(struct translation* t, int child)
{
  ...
  library::stNodeInterpret* orig = t->current;
  library::stNodeInterpret* node = orig + 1;
...
static void translate_branch(struct translation* t, bool args)
{
  ...
  library::stNodeInterpret* orig = t->current;
  library::stNodeInterpret* node = orig + 1;
  library::stNodeInterpret* last = NULL;
...
struct translation* script_node_translate(library::stNodeInterpret* tree)
{
    if (!tree) return NULL;
...
//    if (!tree) return NULL;
    ...
    unsigned N = 0;
  library::stNodeInterpret* dup = tree;
    while (!IsEndOfTree(dup++)) N++;
    
    /* Duplicate the tree, so platform memory stays unmodified. */
    dup = (library::stNodeInterpret*)malloc(sizeof *tree * (N+1));
...
struct translation* script_translate(library::stTreeInterpret* script)
{
  library::stNodeInterpret* tree = (script->node);
