...
static void DrawRoute(struct route route, ImVec2 offset, ImVec2 size, ImDrawList* drawlist)
{
    if (xrayHandle.sourceRecord.numNodes != 0)
    {
        ...
        {
            stVector3D v1 = xrayHandle.sourceRecord.nodes[i].position;
            stVector3D v2 = xrayHandle.sourceRecord.nodes[i + 1].position;
...
static void DisplayRouteWindow()
{
    ...
    if (recording_route)
    {
        ...
        stDynamicsReport* report = (stDynamicsReport*)pointer(dynamics->base.report);
        stSuperObject* obstacle = (stSuperObject*)pointer(report->ground.superObject);
...
//        {
//            stVector3D v = actor_position(actor_rayman);
