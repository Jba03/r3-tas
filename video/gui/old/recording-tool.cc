...
{
    /* Write */
    fwrite(IPT_g_stInputStructure, sizeof(stInputStructure), 1, recording_file);
    ++recording_frame;
}
...
{
    ++recording_frame;
    if (fread(IPT_g_stInputStructure, sizeof(stInputStructure), 1, recording_file) == 0 || recording_frame == n_frames)
    {
        is_playback = false;
        recording_frame = 0;
...
static void display_recording_tool(bool *display)
{
    if (*display && configuration.cheats.enabled)
    {
        ...
        if (is_recording) ImGui::TextColored(ImVec4(0.1f, 0.5f, 1.0f, 1.0f), "Recording frame %d (%.3fmb)", recording_frame, (recording_frame * sizeof(stInputStructure)) / 10e+5);
