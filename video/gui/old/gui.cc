...
#include "imgui_memory_editor.hh"
...
static void draw_rayman_info()
{
    if (actor_rayman)
    {
        const stDynam* dynam = actor_dynam(actor_rayman);
        if (!dynam) return;
        ...
        const stSuperObject* ray = (const stSuperObject*)actor_superobject(actor_rayman);
        if (!ray) return;
        
        const stTransform* T_ray = (const stTransform*)pointer(ray->globalTransform);
        if (T_ray)
        {
            const stMatrix4D M_ray = matrix4_host_byteorder(T_ray->matrix); // Player matrix
            const stVector3D P_ray = game_matrix4_position(M_ray); // Player position
...
#define displacement(v,t) (((v + v) / 2.0f) * t)
            ...
            ImGui::Text("Level end trigger (look) = %.3f°", degrees(fnActorHorizontalSightingRelative(actor_rayman, fnActorGetPosition(actor_changemap))));
...
#pragma mark Behavior
            ...
            if (T_ray)
            {
                const stMatrix4D M_ray = matrix4_host_byteorder(T_ray->matrix); // Player matrix
                const stVector3D P_ray = game_matrix4_position(M_ray); // Player position
                const stSuperObject* current_sector = sector_by_location(father_sector, P_ray);
                if (current_sector)
                {
                    const stSector* sect = (const stSector*)superobject_data(current_sector);
...
//                ImGui::Separator();
                ...
                if (ImGui::Button("Dump raw triangle data")) game_triangle_data_dump();
...
static void draw_hierarchy_list(const stSuperObject* root)
{
...
stVector3D targetPoint;

void gui_render_callback(void* ctx)
{
...
//        memset(xrayHandle.pointset, 0, 65536 * 4 * sizeof(stVector3D));
...
//    drawlist->AddLine(rpos, spos, ImColor(1.0f, 0.0f, 0.0f, 1.0f));
    ...
    if (hierarchy)
    {
        ...
        const stSuperObject* sector = sector_by_location(father_sector, fnActorGetPosition(actor_rayman));
        if (level_geometry_draw) DrawLevelGeometry(level_geometry_draw_all_sectors ? father_sector : sector);
        
        ImGui::Begin("Target");
        ImGui::InputFloat3("position", &targetPoint.x);
        if (ImGui::Button("Copy current")) targetPoint = fnActorGetPosition(actor_rayman);
        ImGui::Text("Angle: %f", degrees(fnActorHorizontalSightingRelative(actor_rayman, targetPoint)));
        ImGui::End();
        
        ImGui::Begin("test");
        if (ImGui::Button("save")) game_memory_dump(); //PushPlayerState(CurrentGameState());
        if (ImGui::Button("load")) {
            char path[4096];
            sprintf(path, "%s/memory.bin", LIBR3TAS_DIR);
            FILE* fp = fopen(path, "rb");
            fread((void*)memoryBase, sizeof(uint8), 24 * 1000 * 1000, fp);
           // fwrite(memoryBase, sizeof(uint8), 24 * 1000 * 1000, fp);
            fclose(fp);
            
        } //RestorePlayerState(LastState());
        if (ImGui::Button("start")) BruteforceBegin();
        if (ImGui::Button("run!")) emulatorRunFunction(0x8003b248, 0x8003748c);
        ImGui::End();
...
//        stSuperObject* soo;
//        const stInstantiatedPhysicalObject* ipo = ipo_find("IPO:IPO_w_e_clairiere", father_sector, &soo);
//        ImGui::Text("IPO: %X\n", offset(ipo));
//        static stVector3D pointa = {};
//        const stMatrix4D T = fnActorGetMatrix(actor_rayman);
//        if (actor_rayman)
//        {
//            const struct stPhysicalObject* po = (const struct stPhysicalObject*)pointer(ipo->stPhysicalObject);
//            if (!po) goto skip;
//            
//            const stPhysicalCollideSet* collset = (const stPhysicalCollideSet*)pointer(po->physical_collideset);
//            if (!collset) goto skip;
//            
//            const stCollideObject* zdr = (const stCollideObject*)pointer(collset->zdr);
//            if (!zdr) goto skip;
//            
...
//        extern stOctreeNode* selected_nodes[OCTREE_MAX_SELECTED_NODES];
//        extern int n_selected_nodes;
//
//        n_selected_nodes = 0;
//        memset(st, 0, sizeof(float) * OCTREE_MAX_SELECTED_NODES);
//        memset(selected_nodes, 0, sizeof(stOctreeNode*) * OCTREE_MAX_SELECTED_NODES);
//
//        struct xray_node a;
//        a.position = actor_position(actor_rayman);
//        a.sector = (stSuperObject*)sector_by_location(father_sector, a.position);
//
//        struct xray_node b;
//        b.position = actor_position(fnFindActor(actor_instance_name, "BEN_Box2", dynamic_world));
//        b.sector = (stSuperObject*)sector_by_location(father_sector, b.position);
//
...
//    ImGui::End();
    ...
    if (hierarchy)
    {
        stInputDevice dev = IPT_g_stInputStructure->device[0];
        stPadReadingOutput *output = (stPadReadingOutput*)(memoryBase + 0x0042f8f8);
        stVector3D v = vector3_host_byteorder(output->globalVector);
        
        //printf("pad: %.2f %.2f %.2f\n", v.x, v.y, v.z);
        //printf("off: %X\n", offset(&IPT_g_stInputStructure->device[2].padReadOutput));
    }
    ...
    //if (hierarchy) drawCollisionPlanes(actor_rayman, off, sz, drawlist);
