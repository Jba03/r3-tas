...
{
    if (IPT_g_stInputStructure && *display)
    {
        ...
        if (ImGui::BeginTable("Input structure", 5, ImGuiTableRowFlags_Headers | ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg))
        {
            ...
            stInputEntryElement* entry = (stInputEntryElement*)pointer(IPT_g_stInputStructure->entries);
            int c = host_byteorder_32(IPT_g_stInputStructure->numEntries);
            while (c-- && entry)
            {
